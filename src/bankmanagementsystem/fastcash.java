 ...
 image.add(exit); 
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==exit){
            ...
            String amountt =((JButton)ae.getSource()).getText().substring(3);
            Conn c = new Conn();
            try{
                ...
                if(ae.getSource()!= exit && balance<Integer.parseInt(amountt)){
                    JOptionPane.showMessageDialog(null,"INSUFFICENT BALANCE");
                    return;
                }
                Date date = new Date();
                String query = "insert into bank values('"+pinno+"','"+date+"','withdrawl','"+amountt+"')";
                c.s.executeUpdate(query);
                JOptionPane.showMessageDialog(null,"RS "+ amountt+" DEBITED FROM YOUR ACCOUNT");
                setVisible(false);
                new transaction(pinno).setVisible(true);
            }catch(Exception e){
